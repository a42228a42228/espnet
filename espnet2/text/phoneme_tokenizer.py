...
g2p_choices = [
    ...
    "pyopenjtalk_prosody_sp_token_with_role",
    "pypinyin_g2p",
    "pypinyin_g2p_phone",
...
def pyopenjtalk_g2p_prosody_sp_token_with_role(text: str, drop_unvoiced_vowels: bool = True) -> List[str]:
    role = ['<0>', '<1>', '<2>']
    if text[:3] in role:
        role_prefix = text[:3]
        clean_text = text[3:]
    else:
        prosody_sp_token = pyopenjtalk_g2p_prosody_sp_token(text, drop_unvoiced_vowels)
        return prosody_sp_token
    prosody_sp_token = pyopenjtalk_g2p_prosody_sp_token(clean_text, drop_unvoiced_vowels)
    prosody_sp_token.insert(0, role_prefix)
    return prosody_sp_token
...
def pyopenjtalk_g2p_prosody_sp_token(text: str, drop_unvoiced_vowels: bool = True) -> List[str]:
    ...
    Example:
        >>> pyopenjtalk_g2p_prosody_sp_token("時刻はまもなく<e>2時</e>5分です")
        (['^', 'j', 'i', ']', 'k', 'o', 'k', 'u', 'w', 'a', '#', 'm', 'a', '[', 'm', 'o', ']', 'n', 'a', 'k', 'u', '#', '<e>', 'n', 'i', ']', 'j', 'i', '</e>', '#', 'g', 'o', ']', 'f', 'u', 'N', 'd', 'e', 's', 'u', '$'])
    """
...
class PhonemeTokenizer(AbsTokenizer):
    ...
    def __init__(
        ...
        elif g2p_type == "pyopenjtalk_prosody_sp_token_with_role":
            self.g2p = pyopenjtalk_g2p_prosody_sp_token_with_role
